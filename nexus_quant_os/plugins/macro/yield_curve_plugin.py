...
logger = logging.getLogger("nexus_quant_os.plugins.macro.yield_curve")


class YieldCurvePlugin(BasePlugin):
    """10年期減2年期公債殖利率利差 (T10Y2Y) 外掛。
    ...
    def __init__(self, api_key: Optional[str] = None) -> None:
        super().__init__("YieldCurvePlugin")
        self.api_key = api_key or os.environ.get("FRED_API_KEY")
        if not self.api_key:
            raise ValueError("必須提供 FRED_API_KEY 以初始化 YieldCurvePlugin")
        self.client = Fred(api_key=self.api_key)

    def fetch(self, start: str, end: str) -> PluginOutput:
        logger.info(f"[{self.name}] 正在獲取 T10Y2Y 殖利率曲線數據: {start} -> {end}")
        
        try:
            # T10Y2Y: 10-Year Treasury Constant Maturity Minus 2-Year Treasury Constant Maturity
            series = self.client.get_series('T10Y2Y')
            ...
            series_filtered = series[
                (series.index >= start_ts - pd.Timedelta(days=30)) & 
                (series.index <= end_ts)
            ].copy()
            ...
            df = pd.DataFrame({
                ...
                'yield_curve_slope': series_filtered.values
            })
