...
def run_backtest(start_date: str | None = None, end_date: str | None = None) -> None:
    ...
    for t in range(1, len(w_smooth)):
        for i in range(len(assets)):
            if limit_reject_mask[t, i]:
                if w_final[t, i] != w_final[t-1, i]:
                    w_final[t, i] = w_final[t-1, i]
                    reject_count += 1
    print(f"    ✓ 一字線拒絕成交已套用 (共觸發 {reject_count} 次)")

    # 計算每日權重變動 (Turnover)
    w_prev = np.vstack([np.zeros((1, w_final.shape[1])), w_final[:-1]])
    w_diff = w_final - w_prev  # [T_val, N_assets]
    ...
    port_ret    = (w_final * y_raw_val[:min_len]).sum(axis=1) + (sqqq_weights * sqqq_val_ret)    # [T_val]
